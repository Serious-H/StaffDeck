...
class _Workspace:
    ...
    def resolve(
        ...
        *,
        ...
        if (
            workspace_relative is not None
            and _TRASH_DIRECTORY in workspace_relative.parts
        ):
            raise HarnessExecutionError(
                "RESERVED_PATH",
        ...
        is_internal_tool_result = (
            ...
            and workspace_relative is not None
            and workspace_relative.parts[:2]
            == (_WORKSPACE_INTERNAL_DIRECTORY, "tool-results")
        )
        if (
            workspace_relative is not None
            and _WORKSPACE_INTERNAL_DIRECTORY in workspace_relative.parts
            and not is_internal_tool_result
        ):
            raise HarnessExecutionError(
                "RESERVED_PATH",
                "The Harness internal workspace directory is not directly accessible.",
                details={"path": workspace_relative.as_posix()},
            )
        if (
            self.context.enforce_task_workspace_layout
            and not is_internal_tool_result
            and (
                workspace_relative is None
                or (
                    workspace_relative.parts
                    and (
                        allowed_roots is None
                        or workspace_relative.parts[0] not in allowed_roots
                    )
                )
            )
        ):
...
def register_file_tools(registry: HarnessRegistry) -> HarnessRegistry:
    ...
    path_contract = (
        "TaskFrame paths use the governed input/, work/, and output/ directories. "
        "Relative paths start in the current TaskFrame workspace. In compatibility "
        "callers that do not enforce the TaskFrame layout, absolute and parent-directory "
        "paths remain subject to host permissions and any enabled OS sandbox. "
    )

...
- 文件工具、exec_command、Skill 脚本和临时脚本统一使用当前 TaskFrame 相对路径；不要输出
  或猜测宿主机路径。每个 TaskFrame 的文件布局固定为：`input/` 是服务端提供的只读输入；
  `work/` 用于临时计划、缓存和中间结果；`output/` 只存最终交付物。不得在 TaskFrame 根目录
  创建业务文件，不得修改 `input/`。只有 `output/` 下的新增或修改文件会被系统发现并
...
- 当你为一次性处理编写并执行临时脚本时，脚本源码中的输入路径和最终输出路径必须使用
  ...
  根据 `os.getcwd()`、宿主机路径或父目录推导。兼容层仍接受旧的 `/workspace/...` 输入，但新
  动作不得生成、返回或硬编码这种别名。脚本本身放在 `work/`；最终文件必须
  明确写到 `output/<文件名>`。如需建目录，只能创建该最终输出文件的父目录或 `work/` 下的目录，
  绝不能创建、删除或写入工作区根目录。执行后先确认退出码为 0 且目标文件存在，再调用
  `publish_artifact`；优先使用 exec_command 返回的 `workspace_changes` 中的实际文件路径，禁止
  猜测输出文件名。脚本失败时先依据 stderr 修正脚本或路径，不要把根目录是否可写作为可反复
  尝试的假设。
...
- HTTP/MCP Tool 的 JSON 结果序列化后不超过 2000 字符时直接返回；更大的结果只返回
  `kind=sandbox_json_file`、`path`、`size` 和 `sha256`，完整内容保存在当前 TaskFrame。
  `path` 是文件工具、exec_command 和临时脚本通用的相对只读路径。需要查看时调用现有
  `read_file`，按其 `next_offset` 继续分段读取；
  不得猜测未读取内容，也不得要求系统生成额外摘要或 Schema。
- 需要对 `sandbox_json_file` 做一次性的复杂筛选、转换或统计时，可以使用 `write_file`
  在 `work/` 创建临时脚本，并把返回的 `path` 写入脚本作为只读输入。`.harness/tool-results/`
  是内部只读结果区：不得对其调用 `copy_file`、
...
- 涉及“今天、昨天、本周、过去 N 天、当前月份、截止时间”等相对日期时，必须先调用
  `current_datetime` 获取可信日期、星期和时区，再计算查询区间；不得依赖模型知识或工具结果中
  偶然出现的日期推断当前时间。
- attachments 中 `materialized=true` 的附件已经由服务端写入当前 TaskFrame 的
  `input/attachments/`；使用附件描述中的 `path` 交给 read_file、exec_command 或脚本。不得猜测
